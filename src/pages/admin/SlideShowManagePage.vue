...
const selectedProjectCourse = ref<{
  label: string;
  value: { projectId: number; courseId: number };
} | null>(null);
...
const columns: QTableColumn[] = [
  ...
  {
    name: 'courseInfo',
    label: 'ข้อมูลหลักสูตรและโครงการ',
    field: 'courseName',
    align: 'left',
    style: 'width: 45%',
  },
...
const openEditDialog = (slide: any) => {
  ...
  const foundOption = projectOptions.value.find(
    (opt) => opt.value.projectId === slide.projectId && opt.value.courseId === slide.courseId,
  );
...
const deleteData = (id: number) => {
  $q.dialog({
    title: 'ยืนยันการลบ',
    message: 'คุณต้องการลบ Slide Show นี้ใช่หรือไม่?',
    cancel: true,
    persistent: true,
  }).onOk(() => {
  ...
  <q-page class="q-pa-md q-pa-lg-xl bg-grey-1">
    <div class="row items-center q-mb-md">
      ...
      <h1 class="text-h5 text-weight-bold text-dark q-my-none tracking-tight">
        จัดการ Slide Show ที่หน้าแรกของระบบ
      </h1>
    </div>
    ...
    <q-card flat bordered class="bg-white" style="border-radius: 12px; border: 1px solid #e0e0e0">
      <div class="bg-grey-2 q-pa-md row items-center" style="border-bottom: 1px solid #e0e0e0">
        ...
        <div class="text-subtitle1 text-weight-bold text-dark">
          ข้อมูล Slide Show ที่หน้าแรกของระบบ
        </div>
      </div>
      ...
      <q-card-section class="q-pa-lg">
        <div class="row justify-between items-center q-mb-md">
          ...
          <div class="col-auto row q-gutter-sm items-center">
            <q-input
              ...
              dense
              v-model="search"
              placeholder="ค้นหาชื่อหลักสูตร หรือชื่อโครงการ"
              style="width: 320px"
              bg-color="white"
            >
            ...
            <q-btn
              ...
              color="green-7"
              icon="add_circle"
              label="เพิ่มข้อมูล Slide Show ใหม่"
              no-caps
        ...
        <q-table
          ...
          table-header-class="bg-grey-1 text-weight-bold text-dark text-center"
        >
          <template v-slot:body-cell-index="props">
            <q-td :props="props" class="text-center"
              ><span class="text-body2 text-grey-8">{{ props.row.index }}</span></q-td
            >
          </template>
          ...
          <template v-slot:body-cell-courseInfo="props">
            <q-td :props="props">
              <div class="column q-py-sm">
                <div
                  class="text-subtitle2 text-weight-bold text-dark q-mb-xs"
                  style="line-height: 1.3"
                >
                  {{ props.row.courseName }}
                </div>
                <div class="text-caption text-green-7 text-weight-medium q-mb-xs">
                  โครงการ: {{ props.row.projectName }}
                </div>
          ...
          <template v-slot:body-cell-status="props">
            <q-td :props="props" class="text-center">
              <q-chip
                clickable
                @click="slideStore.toggleStatus(props.row.id)"
                :color="props.row.isActive ? 'green-7' : 'grey-5'"
                text-color="white"
                size="sm"
      ...
      <q-card style="width: 600px; max-width: 90vw; border-radius: 12px">
        <q-card-section class="row items-center q-pb-none q-mb-md">
          <div class="text-h6 text-weight-bold text-dark">
            <q-icon
              :name="isEdit ? 'edit' : 'add_photo_alternate'"
              color="primary"
              class="q-mr-sm"
            />
        ...
        <q-card-section class="q-pt-md">
          <q-form @submit.prevent="saveData" class="q-gutter-md">
            <div>
              <label class="text-weight-bold q-mb-xs block"
                >เลือกโครงการและหลักสูตร <span class="text-red">*</span></label
              >
              <q-select
                outlined
                dense
                v-model="selectedProjectCourse"
            ...
            <div class="q-mt-md">
              <label class="text-weight-bold q-mb-xs block"
                >ลิงก์รูปภาพแบนเนอร์ (URL) <span class="text-red">*</span></label
              >
              <q-input
                outlined
                dense
